...
require_once DOL_DOCUMENT_ROOT.'/custom/modulename/cheque/class/remisecheque.class.php';
...
if ($action == 'confirm_reject_check' && $confirm == 'yes' && $user->hasRight('banque', 'cheque')) {
    ...
    $reason_rejection = GETPOST('reason_rejet_cheque', 'alpha');

    // Update the cheque rejection logic to save the reason
    $object->fetch($id);
    $paiement_id = $object->rejectCheck($rejected_check, $reject_date);
    if ($paiement_id > 0) {
        // Save the rejection reason
        $sql = "UPDATE llx_paiement SET reason_rejet_cheque = '" . $db->escape($reason_rejection) . "' WHERE rowid = " . (int) $id;
        $db->query($sql);
...
$sql .= $db->order($sortfield, $sortorder);
	...
	if ($resql) {
		...
		print '<tr class="liste_titre">';
		print_liste_field_titre("Chèques", '', '', '', '', 'width="30"');
		...
		print_liste_field_titre("ReasonForRejection", $_SERVER["PHP_SELF"], "p.reason_rejet_cheque", "", $param, "", $sortfield, $sortorder);
		...
		if ($num > 0) {
			while ($objp = $db->fetch_object($resql)) {
				...
				print '<td>'.$objp->reason_rejet_cheque.'</td>';
